...
pub fn thread_names_from_interpreter<I: InterpreterState, P: ProcessMemory>(
    ...
    for modules_dict in candidates {
        for entry in DictIterator::from(process, version, modules_dict)? {
            let (key, value) = entry?;
            ...
            if module_name == "threading" {
                ...
                for i in DictIterator::from(process, version, dictptr)? {
                    ...
                    if name == "_active" {
                        trace!("thread_names_from_interpreter: iterating _active");
                        ...
                        trace!(
                            "thread_names_from_interpreter: resolved {} thread names",
                            ret.len()
                        );
                        break;
                    }
...
fn find_modules_dict<I: InterpreterState, P: ProcessMemory>(
    process: &P,
    version: &Version,
    dict_addr: usize,
) -> Result<usize, Error> {
    for entry in DictIterator::from(process, version, dict_addr)? {
        let (key, value) = entry?;
        let name = copy_string(key as *const I::StringObject, process)?;
        if name == "modules" {
            return Ok(value);
    ...
    Err(format_err!("modules not found in sysdict"))
}
