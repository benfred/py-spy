...
pub fn thread_names_from_interpreter<I: InterpreterState, P: ProcessMemory>(
    interp: &I, process: &P, version: &Version) ->  Result<HashMap<u64, String>, Error> {
                ...
                if name == "_active" {
                    ...
                    for i in DictIterator::from(process, &version, value)? {
                        ...
                        let dict_iter = if thread_type.flags() & PY_TPFLAGS_MANAGED_DICT != 0 {
                            DictIterator::from_managed_dict(process, &version, value, thread.ob_type() as usize)?
                        } else {
...
fn _thread_name_lookup<I: InterpreterState>(spy: &PythonSpy) -> Result<HashMap<u64, String>, Error> {
    ...
    Ok(thread_names_from_interpreter(&interp, &spy.process, &spy.version)?)
}
